document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleSignUp(e) {
        ...
        if (authData.user) {
            ...
            // Objeto que corresponde EXATAMENTE às colunas da tabela
    ...
    function createNewGameDataObject(nomeDeGuerra, pelotao, numerica) {
        ...
        generateQuickQuestsForToday(newGame);
        return newGame;
    }

    // --- FUNÇÕES DE PERSISTÊNCIA (SAVE/LOAD) ---
    async function saveGame() {
        if (!game.player || !game.player.id) return;
        ...
        const { error } = await supabase.from('profiles').update(dataToSave).eq('id', game.player.id);
    ...
    async function loadGame(userId) {
        ...
        if (data && data.full_data) {
            game = data.full_data;
            game.player.id = data.id; // Garante que o ID está no objeto do jogo
            return true;
        }
        return false;
    }
    ...
    async function initGameUI(user) {
        game.player.id = user.id; // Garante que o ID do usuário está no objeto game
        if(!game || !game.player) {
    ...
    async function main() {
        ...
        if (session) {
            ...
            if (success) {
                ...
                console.error("Usuário autenticado, mas perfil não encontrado no banco de dados. Forçando a criação.");
                await handleLogout(); // Desloga para evitar loops
            }
        } else {
