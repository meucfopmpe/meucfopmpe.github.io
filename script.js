document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleSignUp(e) {
        ...
        if (authData.user) {
            const gameData = createNewGameDataObject(nomeDeGuerra, pelotao, numerica);
            calculateInitialState(gameData);
            checkTitleUnlocks(gameData);
            generateQuickQuestsForToday(gameData);
            ...
            const profileToInsert = {
                ...
                profile_pic: gameData.player.profilePic,
                grades_average: 0,
                full_data: gameData 
            };
    ...
    function renderMissionForecast() { const forecastContainer = ui.mainQuestDisplay; forecastContainer.innerHTML = ''; const now = new Date(game.time.currentDate); const quickQuestsContainer = document.createElement('div'); quickQuestsContainer.id = "quick-quests-container"; if(game.missions.quickQuestsToday && game.missions.quickQuestsToday.length > 0){ const catDiv = document.createElement('div'); catDiv.className = 'forecast-category'; catDiv.innerHTML = '<h4>⚡ Missões Diárias</h4>'; const ul = document.createElement('ul'); game.missions.quickQuestsToday.forEach((q, i) => { ul.innerHTML += `<li><label class="${q.completed ? 'completed' : ''}"><input type="checkbox" class="quick-quest-checkbox" data-index="${i}" ${q.completed ? 'checked' : ''}> <span>${q.text}</span></label></li>`; }); catDiv.appendChild(ul); quickQuestsContainer.appendChild(catDiv); } forecastContainer.appendChild(quickQuestsContainer); const missionForecastContainer = document.createElement('div'); missionForecastContainer.id = "mission-forecast-container"; const endOfWeek = new Date(now); endOfWeek.setDate(now.getDate() + (6 - now.getDay())); const endOfNextWeek = new Date(endOfWeek); endOfNextWeek.setDate(endOfWeek.getDate() + 7); const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0); const missions = [...(game.missions.custom || [])]; if(game.missions.activeMain){ let d = new Date(now); for(let i=0; i<game.missions.activeMain.duration; i++){ missions.push({name: `[Principal] ${game.missions.activeMain.name}`, date: d.toISOString().split('T')[0]}); d.setDate(d.getDate()+1); } } const categories = {'Hoje': [], 'Esta Semana': [], 'Próxima Semana': [], 'Este Mês': []}; for(const mission of missions){ const missionDate = new Date(mission.date + 'T00:00:00'); if(mission.completed) continue; const daysRemaining = Math.ceil((missionDate - now) / (1000 * 60 * 60 * 24)); if(daysRemaining < 0 || daysRemaining > 30) continue; const dateInfo = `(Faltam ${daysRemaining} dias - ${formatDate(missionDate)})`; if(missionDate.toDateString() === now.toDateString()) categories['Hoje'].push({...mission, dateInfo}); else if(missionDate > now && missionDate <= endOfWeek) categories['Esta Semana'].push({...mission, dateInfo}); else if(missionDate > endOfNextWeek && missionDate <= endOfNextWeek) categories['Próxima Semana'].push({...mission, dateInfo}); else if(missionDate > endOfNextWeek && missionDate <= endOfMonth) categories['Este Mês'].push({...mission, dateInfo}); } for(const category in categories){ if(categories[category].length > 0){ const catDiv = document.createElement('div'); catDiv.className = 'forecast-category'; catDiv.innerHTML = `<h4>${category}</h4>`; const ul = document.createElement('ul'); categories[category].sort((a,b) => new Date(a.date) - new Date(b.date)).forEach(m => { ul.innerHTML += `<li>${m.name} <span>${m.dateInfo}</span></li>` }); catDiv.appendChild(ul); missionForecastContainer.appendChild(catDiv); } } if(missionForecastContainer.innerHTML === '') missionForecastContainer.innerHTML = '<h4>Nenhuma missão agendada para os próximos 30 dias.</h4>'; forecastContainer.appendChild(missionForecastContainer); }
    function calculateInitialState(targetGame) { const startDate = new Date(targetGame.time.startDate); const currentDate = new Date(targetGame.time.currentDate); const daysDone = Math.max(0, Math.floor((currentDate - startDate) / (1000 * 60 * 60 * 24))); targetGame.player.exp = 0; targetGame.player.level = 1; for (const stat in targetGame.player.stats) { targetGame.player.stats[stat] = { level: 1, exp: 0 }; } if (daysDone <= 0) return; const dailyExp = config.DAILY_TASK_REWARDS; for(let i=0; i < daysDone; i++) {gainMainExp(dailyExp.main_exp * 0.8, false); for(const stat in dailyExp.stats_exp){gainStatExp(stat, dailyExp.stats_exp[stat] * 0.8, false);}} targetGame.player.dailiesCompleted = daysDone; }
    ...
    function generateQuickQuestsForToday(targetGame = game) { targetGame.missions.quickQuestsToday = []; for(let i=0; i<3; i++){ if(Math.random() < config.QUICK_QUEST_CHANCE) { const quest = {...quickQuestPool[Math.floor(Math.random() * quickQuestPool.length)], completed: false}; targetGame.missions.quickQuestsToday.push(quest); } } }
    ...
    async function loadGameAndStart(user) {
        ...
        if (success) {
            ...
            console.error("Usuário autenticado, mas perfil não encontrado. Forçando logout.");
            await handleLogout();
            location.reload();
        }
    }
    ...
    async function main() {
        ...
        if (session) {
            const success = await loadGame(session.user.id);
            if (success) {
                startGame(session.user);
            } else {
                console.error("Usuário autenticado, mas perfil não encontrado. Forçando logout para recomeçar.");
                await handleLogout();
            }
        } else {
